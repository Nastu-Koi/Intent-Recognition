...
async def planner_node(state: OrchestratorState) -> dict:
    ...
    system_msg = SystemMessage(
        content=(
            ...
            "但是低风险只表示不需要风险审批，不表示一定可以直接执行。"
            "当用户上传文件并提出开放式处理请求时，你必须作为 LLM judge 判断："
            "用户是否已经明确说明希望产出什么结果、采用什么处理方式、以及是否有必要的约束。"
            "如果当前意图只能被你猜测为总结/提取/翻译/审阅/改写/转换/问答等某一种，而不是由用户明确表达，"
            "应判定 missing_critical_parameters=true，并触发 clarification human_gate。"
            "不要使用关键词机械匹配；要根据用户表达、文件上下文和可用 Agent 能力进行语义判断。"
            "如果用户目标已经足够明确，即使是只读文档任务，也应直接生成 tasks。"
